...
//Calendar
function eventissimo_frontend_calendar($backcolorHEX,$textColorHEX,$search){
	
	if ($search == TRUE){
		$termSearch = isset($_POST["s1"]) ? $_POST["s1"] : "";
		echo '
	...
	ob_start();
	$arrayCalendarAll = eventissimo_json_events_fullcalendar(0,$termSearch);
	$output = ob_get_contents();
	ob_end_clean();
	
	
	return  eventissimo_stamp_calendar($arrayCalendarAll,$backcolorHEX,$textColorHEX,"month,basicWeek",$termSearch) . 
		"<script>
			jQuery(function() {
					callCalendar();
			});
