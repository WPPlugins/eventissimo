...
function eventissimo_json_events($post_per_page=0,$type='null',$defined="",$order="ASC"){
	...
	foreach ( $loop as $post ) : setup_postdata( $post ); 
		...
		$json_data[] = array(
			...
			"description" => 	nl2br(get_post_meta( $id_events , 'descrizione' , true )),
...
function eventissimo_json_events_fullcalendar($post_per_page=0,$search){
	$json = eventissimo_json_events($post_per_page);
	if ($search!="") {
		$json = eventissimo_searchJson(eventissimo_json_events($post_per_page),"title", $search);
		$response = $json;
	}else {
		$json = eventissimo_json_events($post_per_page);
...
function eventissimo_searchJson($obj, $field, $string) {
    ...
    foreach ( $items as $item ) {
		foreach ( $item as $key => $value) {
			if ((strtoupper($key) == strtoupper($field)) && (strstr(strtoupper($item->$field),strtoupper($string)))) {
				$array_filter[] = $item;
			}
		}
	}
